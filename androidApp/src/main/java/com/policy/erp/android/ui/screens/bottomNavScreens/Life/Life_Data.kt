...
@Composable
fun LifeData(viewModel: ApiViewModel, mainNavController: NavController) {

    val lifePolicyRatesList by viewModel.getGeneralSearchData.collectAsState()

    val lifePolicyRateDatalist by viewModel.getGeneralSearchData.collectAsState()



    LaunchedEffect(lifePolicyRateDatalist) {
        // Update policyRatesList based on filterPolicyRateDatalist
        viewModel.updateFilterGeneralRates(lifePolicyRateDatalist)
    }
    ...
    Scaffold(
        ...
        LazyColumn(Modifier.padding(it)) {
            if (lifePolicyRatesList.isEmpty()) {
                item {
...
//            If there are items, display them in the list
                itemsIndexed(lifePolicyRatesList) { index, item ->
                    if (item != null) {
//                    REOPEN
