package com.policy.erp.android.ui.screens.bottomNavScreens.Health
...
import com.policy.erp.android.ui.screens.PolicyListGeneralView
import com.policy.erp.android.ui.screens.PolicyListView

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun HealthData(viewModel: ApiViewModel, mainNavController: NavController) {

    val healthpolicyRatesList by viewModel.getGeneralSearchData.collectAsState()

    val filterPolicyRateDatalist by viewModel.getFilterGeneralRates.collectAsState()



    LaunchedEffect(filterPolicyRateDatalist) {
        // Update policyRatesList based on filterPolicyRateDatalist
        viewModel.updateFilterGeneralRates(filterPolicyRateDatalist)
    }
    ...
    Scaffold(
        ...
        LazyColumn(Modifier.padding(it)) {
            if (healthpolicyRatesList.isEmpty()) {
                item {
                    Column (
...
//            If there are items, display them in the list
                itemsIndexed(healthpolicyRatesList) { index, item ->
                    if (item != null) {
//                    REOPEN
                        PolicyListGeneralView(item, index + 1, mainNavController = mainNavController)
                    }
                }
            }
        }
