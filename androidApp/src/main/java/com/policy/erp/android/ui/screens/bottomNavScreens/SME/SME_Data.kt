...
@Composable
fun SMEData(viewModel: ApiViewModel, mainNavController: NavController) {

    val smePolicyRatesList by viewModel.getGeneralSearchData.collectAsState()


    val filterSMERateDatalist by viewModel.getGeneralSearchData.collectAsState()



    LaunchedEffect(filterSMERateDatalist) {

        // Update policyRatesList based on filterPolicyRateDatalist
        viewModel.updateFilterGeneralRates(filterSMERateDatalist)
...
//            If there are items, display them in the list
                itemsIndexed(smePolicyRatesList) { index, item ->
                    if (item != null) {
//                    REOPEN
